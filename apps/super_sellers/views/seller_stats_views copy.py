...
class SellerStatViewSet(
    ...
    """
    Routes:
    - GET    /api/sellers/<uuid:pk>/                -> retrieve (overview)
    - GET    /api/sellers/<uuid:pk>/by-event        -> by_event ( par évènement)
    - GET    /api/sellers/<uuid:pk>/stock           -> stock ( stock)
    """
    ...
    @extend_schema(
        ...
        description=(
            "Agrège les ventes **par événement** pour le vendeur.\n\n"
    ...
    @action(detail=True, methods=["get"], url_path="by-event")
    def by_event(self, request, pk=None):
        ...
        paginator, page = self.paginate(request, rows_sorted)
